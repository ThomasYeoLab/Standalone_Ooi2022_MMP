function CBIG_MMP_MultiKRR_workflow_parallel_subfolds( setup_file, save_setup, sub_fold_file, ...
    y_file, covariate_file, feature_mat_file, num_inner_folds, outdir,...
    outstem, test_fold, varargin)

% CBIG_MMP_MultiKRR_workflow_parallel_subfolds( setup_file, save_setup, sub_fold_file, ...
%       y_file, covariate_file, feature_file, num_inner_folds, outdir,...
%       outstem, test_fold, varargin)
% 
% This function was modified from the original
% CBIG_MultiKRR_workflow_parallel_subfolds for the case where the user
% wishes to run the multiKRR in subfolds in parallel.
%
...
%  
%   - feature_mat_file
%     Full path of the file storing a cell array. The cell array contains
...
%
%   - test_fold
%     A scalar specifying which sub fold to run.
%
%   Optional Variables
%
%   - (...'with_bias', with_bias,...) 
...
if(~isempty(setup_file))
    ...
    param_field_names = {'sub_fold', 'y','covariates','feature_mat',...
     ...
     compulsory_variable = {'sub_fold_file', 'y_file','covariate_file','feature_mat_file',...
        'num_inner_folds','outdir','outstem','test_fold'};
...
% save txt file flag to signal completion of step 2
fileID = fopen(fullfile(param.outdir,'step2_completion.txt'),'w');
fprintf(fileID,'# Step 2: All kernels generated');
fclose(fileID);
...
% check whether test_fold is valid
assert(test_fold <= num_test_folds, 'ERROR: test_fold exceeds total number of subfolds')
...
% check if all results there, if yes save final result
finished = 0; 
for n = 1:num_test_folds
    if exist(fullfile(param.outdir,'optimal_acc', strcat('fold_',num2str(n)), ...
            strcat('acc_',param.outstem,'.mat')))
        finished = finished + 1;
    end
end

if finished == num_test_folds
    fprintf('All folds completed.\n')
    for n = 1:num_test_folds
        load(fullfile(param.outdir,'optimal_acc', strcat('fold_',num2str(n)), ...
            strcat('acc_',param.outstem,'.mat')))
        for ker_type = 1: length(param.ker_param)
            if n == 1
                tmp =  opt_pred_acc(ker_type,:);
                acc{ker_type} = tmp;
                optimal_stats{ker_type} = opt_stats_cell{ker_type};
            else
                tmp = opt_pred_acc(ker_type,:);
                acc{ker_type} = [acc{ker_type}; tmp];
                tmp_stats_struct = optimal_stats{ker_type};
                tmp_stats_struct_2 = opt_stats_cell{ker_type};
