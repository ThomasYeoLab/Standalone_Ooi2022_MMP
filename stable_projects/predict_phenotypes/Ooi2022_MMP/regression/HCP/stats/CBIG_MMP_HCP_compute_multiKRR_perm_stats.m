...
for h=1:N_seeds
	...
	for i = 1:length(metrics)
        ...
        load(fullfile(multiKRR_dir, seed, 'results', ['final_result_' outstem '.mat']));
		...
		for i = 1:N_fold
			disp(['fold ' num2str(i)]);
			% load data
			load(fullfile(multiKRR_dir, seed, 'results', 'y', ['fold_' num2str(i)], ['y_regress_' outstem '.mat']));
			load(fullfile(multiKRR_dir, seed, 'results', 'optimal_acc', ['fold_' num2str(i)], ['acc_' outstem '.mat']));
