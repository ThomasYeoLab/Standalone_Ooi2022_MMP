function [pairwise_stats, p_single_models, p_combined_models] = CBIG_MMP_HCP_pairwise_stats_wrapper(outdir)

% [pairwise_stats, p_single_models, p_combined_models] = CBIG_MMP_HCP_pairwise_stats_wrapper
%
...
clear p
m1_acc_vecs = zeros(N_behav,N_folds*N_seeds);
m2_acc_vecs = zeros(N_behav,N_folds*N_seeds);
for i = 1:length(models)
    % get first model accuracies
    outstem_1 = models{i};
    m1_acc_vecs_tmp = CBIG_MMP_HCP_read_model_results(outstem_1, outdir, ...
	N_seeds, N_folds, behav_ind, metric, store_new);
    % reshape into 3 factor scores, and average accuracies for original behaviours
    m1_acc_vecs(1:3,:) = m1_acc_vecs_tmp(59:61,:);
    m1_acc_vecs(4,:) = mean(m1_acc_vecs_tmp(1:58,:));
    
    % get second model accuracies
    for j = 1:length(models)
        if j ~= i
            outstem_2 = models{j};
            m2_acc_vecs_tmp = CBIG_MMP_HCP_read_model_results(outstem_2, outdir, ...
		N_seeds, N_folds, behav_ind, metric, store_new);
	    % reshape into 3 factor scores, and average accuracies for original behaviours
            m2_acc_vecs(1:3,:) = m2_acc_vecs_tmp(59:61,:);
            m2_acc_vecs(4,:) = mean(m2_acc_vecs_tmp(1:58,:));
            
            % get p value
            for k = 1:N_behav
                diff_vec = (m1_acc_vecs(k,:) - m2_acc_vecs(k,:))';
                p(j,k) = CBIG_corrected_resampled_ttest(diff_vec, 1/9, 0); 
            end
        else
            p(j,:) = NaN(1,N_behav);
        end
...
% list of models
models = {'best_fmri_KRR' 'multiKRR_k1_rs_k2_wm_k3_lang_k4_gamb_k5_social_k6_motor'...
...
p_combined_models = [pairwise_stats.best_fmri_KRR_corr(2:4,:); ...
    pairwise_stats.stacking_LRR_rs_wm_lang_gamb_social_motor_corr([2 4],:); ...
    pairwise_stats.best_fmri_KRR_COD(2:4,:); pairwise_stats.stacking_LRR_rs_wm_lang_gamb_social_motor_COD([2 4],:)];
