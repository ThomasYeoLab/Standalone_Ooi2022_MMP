function CBIG_MMP_ABCD_stacking(outerFolds, innerFolds, krr_output_dir, feature_mats_cell, ...
    outstem_name, outdir, ymat, covmat, y_idx, second_lvl)

%function CBIG_MMP_ABCD_stacking(outerFolds, innerFolds, krr_output_dir, feature_mats_cell, ...
%   outstem_name, outdir, ymat, covmat, y_idx, second_lvl)
%
% This function prepares the input parameters for the stacking model using the 
% leave-p-out workflow. User can specify the type of regression to be done for the
% second level.
% 
% Inputs:
%   - outerFolds
...
for fold_idx = 1:num_outer_folds
    ...
    param.outdir = fullfile(outdir, outstem, y_name, fold_name);
    fprintf('--- Outerfold %d... ---\n', fold_idx)
    %% get subfold
    fprintf('[1] Fetch subfold... \n')
    feature_name = feature_mats_cell{1};
    feature_outstem = strcat('KRR_',feature_name);
    subfold = load(fullfile(krr_output_dir,'no_relative_3_fold_sub_list.mat'));
    ...
    for n = 1:size(feature_mats_cell,2)   
        ...
        feat_train_tmp = load(fullfile(krr_output_dir,feature_outstem, ...
	   'results','innerloop_cv',fold_name,feature_result_mat));
        feat_test = load(fullfile(krr_output_dir,feature_outstem,'results', ...
	   'test_cv',fold_name,feature_result_mat));
        feat_opt = load(fullfile(krr_output_dir,feature_outstem,'results', ...
	   strcat('final_result_',feature_outstem,'.mat')));
		...
		feat_train_tmp.y_pred{:,find(lvl1_lambda_set == feat_lambda)}{:,y_idx}(scrambled_idx:last_idx);
         ...
         feat_pred_y(param.sub_fold.fold_index) = ...
		feat_test.y_p{:,find(lvl1_lambda_set == feat_lambda)}{:,y_idx};
