function CBIG_MMP_ABCD_compute_singleKRR_perm_stats(singleKRR_dir, outstem, score_ind, ...
...
if ~exist(result_file,'file')
	...
	load(fullfile(singleKRR_dir, 'results', ['final_result_' outstem '.mat']));
	for i = 1:N_fold
	    disp(['fold ' num2str(i)]);
	    % load data
	    load(fullfile(singleKRR_dir,'results','y', ['fold_' num2str(i)], ['y_regress_' outstem '.mat']));
	    y_resid = y_resid(:,score_ind);
	    opt_lambda = optimal_lambda(i,score_ind);
	    test_ind = sub_fold(i).fold_index;
	    train_ind = ~test_ind;

	    load(fullfile(singleKRR_dir,'results','FSM_test',['fold_' num2str(i)], 'FSM_corr.mat'));
