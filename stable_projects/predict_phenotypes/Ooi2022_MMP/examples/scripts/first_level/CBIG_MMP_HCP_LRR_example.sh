...
# 
# EXAMPLE: 
#	CBIG_MMP_HCP_LRR_example.sh $feature_path $outdir $outerFolds $innerFolds $subtxt $scorecsv \
#		$restrictedcsv $predvar $covtxt $ymat $covmat $split_idx $num_var
# EXAMPLE OF HOW TO CALL FUNCTION:
# 	CBIG_MMP_HCP_LRR_example.sh data_dir/features/rs.mat output_dir 10 10 data_dir/subs.txt \
...
for y_idx in $(seq 1 1 $num_var); do
	...
	matlab -nodesktop -nosplash -nodisplay -r " try addpath('$script_dir'); CBIG_MMP_HCP_LRR_example($outerFolds, \
	   $innerFolds,'$feature_path','$featurebase',$split_idx,'$outdir','$subtxt','$scorecsv', \
