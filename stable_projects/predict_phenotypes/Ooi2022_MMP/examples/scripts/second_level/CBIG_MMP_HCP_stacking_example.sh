...
# 
# EXAMPLE: 
#	CBIG_MMP_HCP_stacking_example.sh $outstem $krr_output_dir $outdir $outerFolds $innerFolds \
#		$ymat $covmat $second_lvl $split_idx $num_var $feature_mat_cells
# EXAMPLE OF HOW TO CALL FUNCTION:
# 	CBIG_MMP_HCP_stacking_example.sh "stacking_LRR" output_dir output_dir 10 10 \
...
for y_idx in $(seq 1 1 $num_var); do
	...
	matlab -nodesktop -nosplash -nodisplay -r " try addpath('$script_dir'); CBIG_MMP_HCP_stacking_example( \
