...
# 
# EXAMPLE: 
#	CBIG_MMP_HCP_multiKRR_example.sh $outstem $mkrr_feature_folder $outdir $outerFolds $innerFolds \
#		$subtxt $scorecsv $restrictedcsv $predvar $covtxt $ymat $split_idx $fold_idx \
#		$feature_mat_cells $kernel_groups
# EXAMPLE OF HOW TO CALL FUNCTION:
# 	CBIG_MMP_HCP_KRR_example.sh "multiKRR_rs_wm" data_dir 10 10 data_dir/subs.txt \
...
matlab -nodesktop -nosplash -nodisplay -r "try addpath('$script_dir'); CBIG_MMP_HCP_multiKRR_example( \
