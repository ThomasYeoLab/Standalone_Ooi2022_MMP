...
%NBS: Computes the network-based statistic (NBS) as described in [1]. 
%
...
for i=1:M
    ...
    tmp.tstat=CBIG_ttest2_stat_only(cmat(i,:),pmat(i,:)); 
    t_stat(i)=tmp.tstat;
end
if strcmp(TAIL,'both')
    t_stat=abs(t_stat);
...
for k=1:K
    ...
    for i=1:M
        ...
        tmp.tstat=CBIG_ttest2_stat_only(d(i,1:nx),d(i,nx+1:nx+ny));
        t_stat_perm(i)=tmp.tstat;
    end
    if strcmp(TAIL,'both')
