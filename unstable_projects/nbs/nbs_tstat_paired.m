function [S, tstat, adj, sz_links, NULL] = nbs_tstat_paired(X,Y,THRESH,K,TAIL)
...
for i = 1:N
    for j = 1:M
        if(i~=j)
            tstat(i,j)= CBIG_ttest2_paired_stat_only(squeeze(X(i,j,:)-Y(i,j,:)));
        end
    end
end
tstat(a == b) = 0; 
...
%    d = [X_reshaped, Y_reshaped];
   indperm = round(rand(1,nx)); %Permuting here
   % Switch member of X and Y (X and Y are paired)
   dX = X_reshaped;
   dY = Y_reshaped;
   dX(:,indperm==0) = Y_reshaped(:,indperm==0);
   dY(:,indperm==0) = X_reshaped(:,indperm==0);
   
   tstat_perm = zeros(size(dX,1),1);
   for i = 1:size(dX,1)
       tstat_perm(i) = CBIG_ttest2_paired_stat_only(squeeze(dX(i,:)-dY(i,:)));
   end
   ...
   [tmp, sz_links_perm] = CBIG_components_subgraphs(tstat_perm_thresh);
