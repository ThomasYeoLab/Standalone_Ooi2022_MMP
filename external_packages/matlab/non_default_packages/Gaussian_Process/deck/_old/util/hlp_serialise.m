...
%                                (C) 2010 Tim Hutt
    ...
    elseif isjava(v)
        warn_once('hlp_serialise:cannot_serialize_java','Cannot properly serialize Java class %s; using a placeholder instead.',class(v));
        m = serialize_string(['<<hlp_serialise: ' class(v) ' unsupported>>']);
    else
        try
            m = serialize_object(v);
        catch
            warn_once('hlp_serialise:unknown_type','Cannot properly serialize object of unknown type "%s"; using a placeholder instead.',class(v));
            m = serialize_string(['<<hlp_serialise: ' class(v) ' unsupported>>']);
        end
    end
end
...
function m = serialize_cell_heterogenous(v)
    contents = cellfun(@dk.util.hlp_serialise,v,'UniformOutput',false);
    ...
    elseif isempty(v)
        ...
        elseif (size1+size2 == 0) & (dims == 2) %#ok<AND2>
            ...
            elseif length(unique(cellfun(@class,v(:),'UniformOutput',false))) == 1
                ...
                m = [uint8(38); dk.util.hlp_serialise(class(v{1})); ndims(v); typecast(uint32(size(v)),'uint8').'];
            else
...
function m = serialize_object(v)
    try
        ...
        if isstruct(conts) || iscell(conts) || isnumeric(conts) || ischar(conts) || islogical(conts) || isa(conts,'function_handle')
            ...
            conts = dk.util.hlp_serialise(conts);
        else
...
function m = serialize_handle(v)    
    ...
    switch rep.type
        ...
        case 'anonymous'
            ...
            if isfield(tracking,'serialize_anonymous_fully') && tracking.serialize_anonymous_fully
                ...
                if length(m) > 2^18
                    ...
                    warn_once('hlp_serialise:large_handle','The function handle with code %s references variables of more than 256k bytes; this is likely very slow.',rep.function); 
                end
            else
        ...
        otherwise
            ...
            m = serialize_string(['<<hlp_serialise: function handle of type ' rep.type ' unsupported>>']);
    end
end
