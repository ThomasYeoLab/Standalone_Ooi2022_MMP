function ts_out = ft_bandpass( ts_in, fband, varargin )
%
% Adapted from FieldTrip.
    ...
    switch type
        ...
        case {'but','butter'}
            ...
            hfilt = @(ord) butter( ord, fband/fn, 'bandpass' );
            
        case 'fir'
            
            order = min( 3*fix(fs/fband(1)), floor((nt-1)/3) );
            order = opt.get('order',order);
            hfilt = @(ord) fir1( ord, fband/fn, 'bandpass' );
        ...
        case 'firws'
            ...
            hfilt = @(ord) ant.priv.ft_firws( ord, fband/fn, 'band', win );
            
        otherwise
            error('[ant.dsp.ft_bandpass] Unsupported filter type "%s".',type);
    end
