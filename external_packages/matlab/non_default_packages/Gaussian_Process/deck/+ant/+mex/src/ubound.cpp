...
void usage() {
    jmx::println("Usage [find upper-bound along specified dimension]:");
    jmx::println("    index = ubound( vals, query )");
    jmx::println("where");
    jmx::println("    vals  = nxd matrix");
    jmx::println("    query = 1xd vector of threshold");
    jmx::println("    index = 1xd vector of indices\n");
    jmx::println("For each row or column, find index of upper-bound.");
    ...
    int nargin, const mxArray *in[] )
{
    ...
    for ( index_t k = 0; k < nd; k++ ) { // iterate over columns of Val
        ...
        auto bound = std::upper_bound( first, last, Qry[k] );
