...
pushd bold
   ...
   while ($k <= ${#zpdbold})
	pushd $zpdbold[$k]
	    zero_lt_4dfp 50 $subject"_bld"$zpdbold[$k]_$runid[$k]"_"$ppstr $subject"_bld"$zpdbold[$k]_$runid[$k]"_"$ppstr"_mskt"
	popd
	@ k++
    end
popd
...
@ k = 1
while ($k <= ${#zpdbold})
	echo bold/$zpdbold[$k]/$subject"_bld"$zpdbold[$k]_$runid[$k]"_"$ppstr"_mskt" >> $lst
	@ k++
end
