...
set idstr = '$Id$'
#########################################################################################
# This script runs the fcMRI preprocessing (which follows initial preprocessing).	#
# No whole-brain signal regression will be performed. Instead, regressors consist of 	#
# motion parameters (+derivatives) and 5 CompCor components (from vent+wm)		#
# Subject-specific parameters are read in scripts/$subject/params.			#
#											#
# Modified from $CODE_DIR/lib/shell/fcMRI_preproc_nifti.csh by Jesisca Tandi		#
#########################################################################################
...
# Use aCompCor according to Chai et al., 2012
########################################################################
VENT_WM:
# Combine VENT and WM mask
set wm_vent = `dirname $ventreg`/${subject}.func.wm.vent.nii.gz
fslmaths $wmreg -max $ventreg ${wm_vent}
set cmd = ( matlab -nojvm -nosplash -nodesktop -r '"' 'addpath(genpath('"'"${CODE_DIR}'/procsurffast/CompCorSurface'"'"'))' ; aCompCor_Gab_multipleruns ${subject}"_"${ppstr}"_"${blurstr}"_bpss".txt ${wm_vent} ${subject}"_vent_wm_5aCompCor".dat'"' )
#set cmd = (matlab -nojvm -nosplash -nodesktop -r "'" 'addpath(genpath(''${CODE_DIR}''/procsurffast/CompCorSurface));' aCompCor_Gab_multipleruns ${subject}"_"${ppstr}"_"${blurstr}"_bpss".txt ${wm_vent} ${subject}"_vent_wm_5aCompCor".dat"'")
echo $cmd
eval $cmd
...
###############################
PASTE:
paste $subject"_mov_regressor".dat $subject"_vent_wm_5aCompCor".dat >! $subject"_regressors".dat	
