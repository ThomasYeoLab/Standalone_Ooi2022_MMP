...
set version = '$Id: procsurffast.csh, v1.0 2011/03/31 ythomas$'
...
## --- run procfast/fcfast
## -----------------------
echo "========= ** procfast/fcfast ** ====================================="
if(! $skipproc) then
    if($lowmem_fast) then
	### --- run procfast with lowmem option
        set cmd = (procfast -sdir $sdir -id $subject -bold $bold -tr $tr -mpr $mpr -stackcheck -nocleanup -lowmem)
	...
	if($matlab_qc_plot) then
        ...
        echo "Running procfast with lowmem option ... Suppressing procfast print out ... Check $sdir/$subject/logs/procfast_lowmem.log for procfast output ..."
    else   
	if($fcfast_flag) then  
	    ### --- run fcfast
	    set cmd = (fcfast -sdir $sdir -id $subject -bold $bold -tr $tr -mpr $mpr -stackcheck -nocleanup)
	    ...
	    echo "Running fcfast ... Suppressing fcfast print out ... Check $sdir/$subject/logs/fcfast.log for fcfast output ..."
	else 
	    ### --- run procfast
	    set cmd = (procfast -sdir $sdir -id $subject -bold $bold -tr $tr -mpr $mpr -stackcheck -nocleanup)
	    ...
	    echo "Running procfast ... Suppressing procfast print out ... Check $sdir/$subject/logs/procfast.log for procfast output ..."
	endif
    endif
    ...
    if($?) then
	echo "ERROR: procfast/fcfast terminates with errors"
	exit 1;
    endif
    ...
    echo "Skipping procfast/fcfast ... Assume procfast/fcfast output at $sdir/$subject"
endif
echo 
...
echo
echo "======== ** Running procsurffast.csh with the following parameters ** ============="

set variable_set = (root_dir subject rawdir anat_rawdir sdir tr mpr bold roilist procsmooth surfsmooth fsresolution fsprojresolution lh_roi rh_roi \
                    ...
                    lowmem_fast matlab_qc_plot intrasub_best_reg skip extract_frames_aft_bbreg_txt cleanup)
...
echo "========= ** Running procsurffast.csh with the following software ** =============="
...
set function_set = (fcfast procfast fsl_preprocess.sh flirt fcMRI_preproc_nifti.csh ComputeROIs2ROIsCorrelationWithRegression) 
...
while( $#argv != 0 )
  ...
  switch($flag)
    ...
    case "-procfast":
      set fcfast_flag = 0; # run procfast instead
      breaksw;
    ...
    case "-skipproc":
      set skipproc = 1; # fcfast/procfast has already been run
      breaksw;  
...
# For procfast/fcfast to occur, need to specify either subject or rawdir
if($skipproc == 0 & (! $?subject) &  (! $?rawdir) ) then
    echo "ERROR: For procfast/fcfast to occur, need to specify either subject or rawdir!"
    exit 1;
endif

# For procfast/fcfast to occur, need to specify tr, mpr, bold
if($skipproc == 0) then
    set variable_set = (tr mpr bold)
    foreach var ($variable_set)
	eval 'set isset = $?'$var
	if(! $isset) then  
	    echo "ERROR: For procfast/fcfast to occur, need to specify $var"
	    exit 1;
	endif
    end 
endif
...
# check if smoothing is set for procfast
if($skipproc == 0 & $fcfast_flag == 0 & (! $?proc_smooth)) then
    echo "WARNING: proc_smooth not set for procfast. So no smoothing is going to take place"
endif
...
usage_exit:
  ...
  echo "                                      can be found if procfast/fcfast has already been run. In particular,"
  echo "                                      processed data is assumed to be at or saved at <sdir>/<subject>."
  echo "                                      [optional; defaults to current directory]"
  echo "                                      IMPORTANT: Note that if <rawdir> = <sdir>/<subject>/RAW, then procfast/fcfast will delete your RAW data!"
  echo "                                      IMPORTANT: Note that if <anat_rawdir> = <sdir>/<subject>/RAW_ANAT, then procfast/fcfast will delete your RAW ANAT data!"
  echo "  -tr <tr>                          : specify sampling rate in sec [optional if skipping procfast/fcfast]"
  echo "  -mpr <mpr>                        : specify t1 weighted mprage number [optional if skipping procfast/fcfast]"
  echo "  -bold <bold_runs>                 : specify bold run numbers followed by a comma (e.g. -bold 5,6,13) [optional if skipping procfast/fcfast]"
  echo "  -roilist <roi_list>               : specify seed regions in a text list [optional if using fcfast; not needed otherwise]"i
  ...
  echo "                                      space for procfast. Not used by fcfast. fcfast smoothing is fixed to 6mm"
  echo "                                      [optional for procfast; not needed otherwise]"
  ...
  echo " "
  echo " "
  echo "  -procfast                         : run procfast instead of fcfast [optional; default runs fcfast]"  
  echo "  -skipproc                         : skip both fcfast and procfast. Processed data is assumed to be in <sdir>/<subject>"
  echo "                                      Assume procfast/fcfast has been run with nocleanup option"
  ...
  echo "                                      specified, procsurffast will run freesurfer on the data, and outputs to <sdir>/<subject>_FS/"
  echo "                                      Therefore anat_dir should point to the equivalent of <sdir>/<s>_FS [optional]" 
  echo "  -lowmem_fast                      : This disables fcMRI preprocessing in the MNI152 space as defined by fcfast; instead run procfast with lowmem option"
...
help_usage:
  ...
  echo "      (1) MNI152 space as defined by fcfast/procfast"
  ...
  echo "      (1) Runs fcfast on resting data. "
  echo "                user can also request procfast instead if processing task-based fMRI (using -procfast flag)."
  ...
  echo "      (4) Perform fcMRI preprocessing on motion corrected fMRI data (under fcfast mode)"
  ...
  echo "EXAMPLE USAGE:"
  echo " "
  echo "  procsurffast.csh -s 100401_HW84YH -tr 3 -mpr 5 -bold 14,15"
  echo "  procsurffast.csh -s 100401_HW84YH -tr 3 -mpr 5 -bold 14,15 -roilist <roi_list>"
  echo "       -- provides fcfast with roi_list for functional connectivity (see fcfast for usage of this option)." 
  echo "  procsurffast.csh -s 100401_HW84YH -tr 3 -mpr 5 -bold 14,15 -lh_roi $_HVD_CODE_DIR/ComputeROIs2ROIsCorrelationWithRegression/Yeo2011_surface_seeds/lh.surface_seeds.txt"
  echo "       -- performs whole cortex (left and right hemi) fcMRI using left hemisphere seed regions as specified in text files" 
  echo "  procsurffast.csh -s 100401_HW84YH -tr 3 -mpr 5 -bold 14,15 -procfast"
  echo "       -- run procfast instead of fcfast"
  echo "       -- motion-corrected fMRI projected instead of fcMRI processed fMRI"
  echo "  procsurffast.csh -s 100401_HW84YH -tr 3 -mpr 5 -bold 14,15 -sdir yada/yada -skipproc"
  echo "       -- assumes fcfast has been already been run with nocleanup option"
  echo "       -- assumes fcfast output is in yada/yada/100401_HW84YH"
  echo "  procsurffast.csh -s 100401_HW84YH -tr 3 -mpr 5 -bold 14,15 -anat_dir foo/bar"
  ...
  echo "  procsurffast.csh -s 100401_HW84YH -tr 3 -mpr 5 -bold 14,15 -volproj"
  ...
  echo "  procsurffast.csh -s 100401_HW84YH -tr 3 -mpr 5 -bold 14,15 -volproj_lowmem"
  echo "       -- also projects data to freesurfer nonlinear volumetric space and FSL MNI152 space via FreeSurfer nonlinear volumetric space"
  echo " "
  echo " "
