...
# checks!
if($#argv < 4) then
    echo "usage: ClusterfcMRISurf2SurfProfiles.csh lh_profile_txt rh_profile_txt num_clusters output_file"
    echo ""
    exit
endif     
...
$MATLAB -nojvm -nodesktop -nosplash -r "bty_pwd = pwd; CODE_DIR = getenv('_HVD_CODE_DIR'); cd(fullfile(CODE_DIR, 'bin')); ythomas_generic_startup; cd(bty_pwd); VonmisesSeriesClustering('$mesh', '$mask', '$num_clusters', '$output_file', '${lh_avg_profile}', '${rh_avg_profile}', '${smooth}', '${num_tries}', '${znorm}'); exit;"
...
if($num_clusters == 17) then
    ...
    $MATLAB -nojvm -nodesktop -nosplash -r "bty_pwd = pwd; CODE_DIR = getenv('_HVD_CODE_DIR'); cd(fullfile(CODE_DIR, 'bin')); ythomas_generic_startup; cd(bty_pwd); HungarianClusterMatchSurfWrapper('$ref_file', '$output_file', '$output_file')"
