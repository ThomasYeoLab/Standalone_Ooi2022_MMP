/*$Header: /data/petsun4/data1/src_solaris/qnt_4dfp/RCS/qnt_4dfp.c,v 1.14 2006/09/25 03:22:40 avi Exp $*/
/*$Log: qnt_4dfp.c,v $
...
/***************************/   
    ...
    short vv, t; 
    char filename[MAXL]; 
    char *buffer[MAXL], *outbuffer, *sbuffer, *mskbuffer;
...
/********************/
char	program[MAXL];
static char rcsid[]= "$Id: qnt_nifti.c,v 1.15 2006/09/25 03:22:40 avi Exp $";
...
/********************************************/
   ...
   for (filenum = 0; filenum < lLineCount ; filenum++){
	...
	nframes += v_dim[filenum];
   	buffer[filenum] = malloc(x_dim[filenum]*y_dim[filenum]*z_dim[filenum]*v_dim[filenum]*bpp);
  	FslReadVolumes(src, buffer[filenum], v_dim[filenum]);
	FslClose(src);
	}
...
/***********************************************/
	...
	vv=0;
	for (filenum = 0; filenum < lLineCount; filenum++){
 	memcpy(outbuffer+X*Y*Z*vv*bpp,buffer[filenum],X*Y*Z*v_dim[filenum]*bpp);
      	vv+=v_dim[filenum];
    	}
...
/*	if (list_Flag == 0) printf ("#Reading: %s\n", imgroot);
	...
	for (iframe = pdefined = ifile = 0; ifile < nfile; ifile++) {
		...
		for (k = 0; k < V_DIM; k++, iframe++) {
			...
			imgt[i] = (float) vol[i + k*dimension + ifile*dimension*V_DIM];
...
/*	if (conc_flag) {
	...
	fclose (mskfp);*/
	free (imgt); free (imgm); 
	free (vol); free (mskvol); free (outbuffer); free (sbuffer); free (mskbuffer); 
	exit (status);
}
