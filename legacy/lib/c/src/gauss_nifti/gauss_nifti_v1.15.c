/*$Header: /data/petsun4/data1/src_solaris/gauss_4dfp/RCS/gauss_4dfp.c,v 1.15 2007/04/23 02:37:19 avi Exp $*/
/*$Log: gauss_4dfp.c,v $
...
/***************************/   
    ...
    short vv, t; 
    char filename[MAXL]; 
    char *buffer[MAXL], *outbuffer, *sbuffer, *coeffbuffer;
...
static char rcsid[] = "$Id: gauss_nifti.c,v 1.15 2007/04/23 02:37:19 avi Exp $";
...
/*	IFH		ifh;*/
	...
	char            text_File[MAXL];
        ...
        float	 	voxdim[3];	
	float		cmppix[3], f0;
	float		*imgt, *imgp, *splitimgt, *imgtbuffer, *fptr;
	int		nx, ny, nz;
	int		nxp, nyp, nzp;
	int		margin, vdim;
	char		control = '\0';
...
/********************/
	...
	imgtbuffer =  (float *) calloc (vdim*V, sizeof (float));	/* frame buffer V added for nifti files */
	if (!imgp || !imgt) printf ("%s cannot allocate memory\n", program); /*errm (program);*/
	...
	if (list_Flag == 1) {
		...
		splitimgt = (float *) calloc (vdim*V_DIM, sizeof (float));
		
		    for (k = i = 0; i < V_DIM; i++) {
			
			for (jndex = 0; jndex < vdim; jndex++){
			    splitimgt[jndex + i*vdim] = imgtbuffer[jndex + i*vdim + file*vdim*V_DIM];
		        }
                    }
             	fptr = splitimgt;
