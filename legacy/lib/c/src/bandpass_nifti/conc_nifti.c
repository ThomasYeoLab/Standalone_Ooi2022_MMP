/*$Header: /data/petsun4/data1/src_solaris/actmapf_4dfp/RCS/conc_nifti.c,v 2.0 2007/03/22
04:54:45 avi Exp $*/
/*$Log: conc_nifti.c,v $*/
...
/* global variables */
/********************/
static char        rcsid[] = "$Id: conc_nifti.c,v 1.8 2006/09/25 04:54:45 avi Exp $";
...
void conc_open_quiet (CONC_BLOCK *conc_block, char *lstfile) {
        char                *ptr, string[MAXL], imgroot[MAXL], *srgv[MAXL];
        int                i, k, status = 0;
        IFH                ifh;
	strcpy (conc_block->lsroot, lstfile)
	...
	printf("Program %s cannot read file \n",conc_block->program, conc_block->lstfile);
        while (fgets (string, MAXL, conc_block->lstfp)) {
                if (!(ptr = strstr (string, "number_of_files:"))) continue;
                conc_block->rnfile = atoi (ptr + 16);
        }
        if (!(conc_block->imgfile0 = (char **) malloc (conc_block->rnfile * sizeof (char
*)))) printf("Memory allocation failure\n");
        if (!(conc_block->nvol     = (int *)   malloc (conc_block->rnfile * sizeof (int))))
   printf("Memory allocation failure\n");
        for (i = 0; i < conc_block->rnfile; i++) {
                if (!(conc_block->imgfile0[i] = (char *) malloc (MAXL * sizeof (char)))) printf("Memory allocation failure\n");
        }
        ...
        for (conc_block->imgdim[3] = i = 0; i < conc_block->rnfile; i++) {
                if (Getifh (conc_block->imgfile0[i], &ifh)) errr (conc_block->program,
...
void conc_open (CONC_BLOCK *conc_block, char *lstfile) {
        ...
        getroot (lstfile, conc_block->lstroot);
        sprintf (conc_block->lstfile, "%s.conc", conc_block->lstroot);
        if (!(conc_block->lstfp = fopen (conc_block->lstfile, "r"))) errr
(conc_block->program, conc_block->lstfile);
        while (fgets (string, MAXL, conc_block->lstfp)) {
                if (!(ptr = strstr (string, "number_of_files:"))) continue;
                conc_block->rnfile = atoi (ptr + 16);
...
*)))) errm (conc_block->program);
   ...
   errm (conc_block->program);
        for (i = 0; i < conc_block->rnfile; i++) {
                if (!(conc_block->imgfile0[i] = (char *) malloc (MAXL * sizeof (char)))) errm
(conc_block->program);
        }
        rewind (conc_block->lstfp);
        i = 0; while (fgets (string, MAXL, conc_block->lstfp)) {
...
conc_block->imgfile0[i]);
        ...
        printf ("imgdim %10d%10d%10d%10d\n",
                        conc_block->imgdim[0], conc_block->imgdim[1], conc_block->imgdim[2],
conc_block->imgdim[3]);
...
conc_block->program);
        ...
        if (!(conc_block->imgfile1 = (char **) malloc (conc_block->wnfile * sizeof (char
*)))) errm (conc_block->program);
        sprintf (conc_block->outfile, "%s_%s.conc", conc_block->lstroot, trailer);
        if (!(conc_block->outfp = fopen (conc_block->outfile, "w"))) errw
(conc_block->program, conc_block->outfile);
        fprintf (conc_block->outfp, "number_of_files: %d\n", conc_block->wnfile);
        for (i = 0; i < conc_block->wnfile; i++) {
                if (!(conc_block->imgfile1[i] = (char *) malloc (MAXL * sizeof (char)))) errm
(conc_block->program);
