/*$Header: /autofs/space/nexus_001/users/nexus-tools/cvsrepository/nifti_tools/var_nifti/var_nifti.c,v 1.2 2010/03/23 15:59:30 mtt24 Exp $*/
/*$Log: var_nifti.c,v $
/*Revision 1.2  2010/03/23 15:59:30  mtt24
/*included memory allocation failure checks
/*
...
/* global variables */
/********************/
static char rcsid[] = "$Id: var_nifti.c,v 1.2 2010/03/23 15:59:30 mtt24 Exp $";
...
float **calloc_float2 (int n1, int n2) {
	...
	if (!(a = (float **) malloc (n1 * sizeof (float *)))) {fprintf (stderr, "%s: could not allocate memory\n", program); exit (-1);}
	if (!(a[0] = (float *) calloc (n1 * n2, sizeof (float)))) {fprintf (stderr, "%s: could not allocate memory\n", program); exit (-1);}
...
/*******************/
	...
	int		imgdim[4], vdim, dimension, nf_total, nf_func, nf_anat = 0;
	int		nfile, ifile;
	int		isbig;
	char		control = '\0';
	float *splitimgt;
...
/***********************************/
	if (strlen (format)) {
		...
		for (k = ifile = 0; ifile < nfile; ifile++) {
			...
			if (nf_func < 0) {
				...
				(list_Flag) ? input_file[ifile] : filename);
			}
	...
	||  !(imgv = (float *) calloc (dimension,  sizeof (float)))) {fprintf (stderr, "%s: could not allocate memory\n", program); exit (-1);}
...
/********************************/
	for (ifile = 0; ifile < nfile; ifile++) {
		...
		if (!(buffer[ifile] = malloc(x_dim[ifile]*y_dim[ifile]*z_dim[ifile]*v_dim[ifile]*bpp))) {fprintf (stderr, "%s: could not allocate memory for buffer\n", program); exit (-1);};
  		FslReadVolumes(src, buffer[ifile], v_dim[ifile]);
		FslClose(src);
		...
		else if (list_Flag == 0) printf ("Reading: %s", filename);
...
/********************/
	...
	for (ifile = 0; ifile < nfile; ifile++) {
		...
		/*buffer[ifile] = malloc(x_dim[ifile]*y_dim[ifile]*z_dim[ifile]*v_dim[ifile]*bpp);*/
  		FslReadVolumes(src, buffer[ifile], v_dim[ifile]);
		FslClose(src);
		if (!(vol = (double *) calloc (vdim*v_dim[ifile], sizeof (double)))) {fprintf (stderr, "%s: could not allocate memory for vol\n", program); exit (-1);}
  		convertBufferToScaledDouble(vol,buffer[ifile],X*Y*Z*v_dim[ifile],1.0,0.0,src->niftiptr->datatype);
...
/****************/
	...
	case 'm':
		...
		for (ifile = 0; ifile < nfile; ifile++) {
			...
			if (list_Flag == 1){
			if (!(splitimgt = (float *) calloc (dimension*v_dim[ifile], sizeof (float)))){fprintf (stderr, "%s: could not allocate memory for splitimgt\n", program); exit (-1);}
			}

			else if (list_Flag == 0){
			if (!(splitimgt = (float *) calloc (dimension*V, sizeof (float)))){fprintf (stderr, "%s: could not allocate memory for splitimgt\n", program); exit (-1);}
			}
			sprintf (listroot[ifile], "%s_%s", output_file[ifile], "uout");
			printf ("Writing: %s.nii.gz", (list_Flag) ? listroot[ifile] : outroot);
			printf ("\t%d frames\n", (list_Flag) ? v_dim[ifile] : imgdim[3]);
			for (j = 0; j < ((list_Flag) ? v_dim[ifile] : imgdim[3]); j++) {
	...
	case 'z': case 'v': case 's':
		imgdim[3] = 1;
		sprintf (outfile, "%s.nii.gz", outroot);
		printf ("Writing: %s\n", outfile);
