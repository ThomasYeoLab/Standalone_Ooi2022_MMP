classdef test_MARS_linearInterpolate_kdTree < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testLHCase(testCase)
            ...
            ref_dir = fullfile(CBIG_CODE_DIR, 'unit_tests', 'external_packages', 'SD', 'SDv1.5.1-svn593', ...
                'BasicTools', 'MARS_linearInterpolate_kdTree');
            ...
            [vals_test, NViF_test, NF_test] = MARS_linearInterpolate_kdTree(points, mesh_input, data);
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results for test_MARS_linearInterpolate_kdTree, testLHCase...');
        ...
        function testRHCase(testCase)
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results for test_MARS_linearInterpolate_kdTree, testRHCase...');
                abserror = abs(vals_test - vals);
                disp(['Total error (vals): ' num2str(sum(sum(abserror)))]);
                abserror = abs(NViF_test - NViF);
                disp(['Total error (NViF): ' num2str(sum(sum(abserror)))]);
