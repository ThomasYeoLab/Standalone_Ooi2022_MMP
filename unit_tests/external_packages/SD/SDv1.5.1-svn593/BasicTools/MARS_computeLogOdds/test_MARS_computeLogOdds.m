    ...
    methods (Test)
        function test_lh_Case(testCase)
            ...
            if replace_unit_test
                % display differences
                disp("Replacing unit test for MARS_computeLogOdds, test_lh_Case");
                ...
                disp(['Old norm prob mat size is [' num2str(size(ref_normalized_prob_mat)) ']']);
                disp(['New norm prob mat size is [' num2str(size(normalized_prob_mat)) ']']);
                ...
                ref_LogOdds = LogOdds;
                ref_normalized_prob_mat = normalized_prob_mat;
                
                save(fullfile(ReferenceDir, 'ref_dist_transform_lh.mat'), 'ref_dist_transform'); % load in ref_dist_transform
                save(fullfile(ReferenceDir, 'ref_LogOdds_lh.mat'), 'ref_LogOdds'); % load in ref_LogOdds
                save(fullfile(ReferenceDir, 'ref_normalized_prob_mat_lh.mat'), 'ref_normalized_prob_mat'); % load in ref_normalized_prob_mat
        ...
        function test_rh_Case(testCase)
            ...
            if replace_unit_test
                % display differences
                disp("Replacing unit test for MARS_computeLogOdds, test_rh_Case");
                disp(['Dist transform diffference is ' num2str(all(all(abs(dist_transform - ref_dist_transform)))) ] );
                disp(['LogOdds difference is ' num2str(all(all(abs(LogOdds - ref_LogOdds)))) ]);
                disp(['Old norm prob mat size is [' num2str(size(ref_normalized_prob_mat)) ']']);
                disp(['New norm prob mat size is [' num2str(size(normalized_prob_mat)) ']']);
                ...
                save(fullfile(ReferenceDir, 'ref_dist_transform_rh.mat'), 'ref_dist_transform'); % load in ref_dist_transform
                save(fullfile(ReferenceDir, 'ref_LogOdds_rh.mat'), 'ref_LogOdds'); % load in ref_LogOdds
                save(fullfile(ReferenceDir, 'ref_normalized_prob_mat_rh.mat'), 'ref_normalized_prob_mat'); % load in ref_normalized_prob_mat
