classdef test_CBIG_KRR_workflow_LITE < matlab.unittest.TestCase
% Written by Jingwei Li and CBIG under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md
    ...
    methods (Test)
        function test_setup_input( TestCase )
            ...
            % param.wigth_bias = 1, so that the beta term is included in the model
            ...
            parent_dir = fullfile(getenv('CBIG_CODE_DIR'), 'unit_tests', 'utilities', 'matlab', ...
                'predictive_models', 'KernelRidgeRegression', 'CBIG_KRR_workflow_LITE');
        ...
        function test_varargin_input_withNaN( TestCase )
            ...
            if replace_unittest_flag
                ...
                mkdir(ref_outdir)
                CBIG_KRR_workflow_LITE( [], 0, sub_fold_file, y_file, covariate_file, feature_file, ...
        ...
        function test_no_save_kernel( TestCase )
            % The inputs are passed in through a single setup struct;
            % param.save_kernel = 0, so that the kernel will not be saved
            % No NaN in the behavior;
            % param.with_bias = 1, so that the beta term is included in the model
            ...
            param.covariates = covariates;
            param.num_inner_folds = 5;
            param.outdir = fullfile(parent_dir, 'output', 'test_no_save_kernel');
            param.outstem = '2cog';
            param.with_bias = 1;
            param.ker_param.type = 'corr';
            param.ker_param.scale = nan;
            ...
            if replace_unittest_flag
                %% replace reference result   
                ref_outdir = fullfile(parent_dir, 'ref_output', 'test_no_save_kernel');
                mkdir(ref_outdir)
				CBIG_KRR_workflow_LITE(param)
				...
				%% compare results
				ref_dir = fullfile(parent_dir, 'ref_output', 'test_no_save_kernel');
				ref = load(fullfile(ref_dir, 'final_result_2cog.mat'));
				test = load(fullfile(param.outdir, 'final_result_2cog.mat'));
				fields = fieldnames(ref);
