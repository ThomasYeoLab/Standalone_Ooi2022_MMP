classdef test_CBIG_DrawSurfaceMaps_fslr < matlab.unittest.TestCase
% Written by Nanbo Sun, Zhang Shaoshi and CBIG under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md
    ...
    methods (Test)
        function test_metric_data_32k_inflated(testCase)
            ...
            cur_dir = fullfile(CBIG_CODE_DIR, 'unit_tests', 'utilities', 'matlab', 'figure_utilities', 'CBIG_DrawSurfaceMaps_fslr');
            ...
            h = CBIG_DrawSurfaceMaps_fslr(lh_fsLR_32k_data, rh_fsLR_32k_data, fslr_mesh_name, surf_type, min_thresh, max_thresh);
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results fo DrawSurfaceMaps_fslr, metric_data_32k_inflated...')
        ...
        function test_metric_data_32k_veryinflated(testCase)
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results fo DrawSurfaceMaps_fslr, metric_data_32k_veryInflated...')
                disp(['Correlation:' num2str(cor)]);
                copyfile(fullfile(cur_dir, 'output', out_name), fullfile(cur_dir, 'ref_output', out_name))
                delete(fullfile(cur_dir, 'output', out_name))
            else           
        ...
        function test_metric_data_32k_midthickness_orig(testCase)
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results fo DrawSurfaceMaps_fslr, metric_data_32k_midthickness_orig...')
                disp(['Correlation:' num2str(cor)]);
                copyfile(fullfile(cur_dir, 'output', out_name), fullfile(cur_dir, 'ref_output', out_name))
                delete(fullfile(cur_dir, 'output', out_name))
            else            
        ...
        function test_label_data(testCase)
            ...
            h = CBIG_DrawSurfaceMaps_fslr(lh_fsLR_32k_data, rh_fsLR_32k_data, mesh_name, surf_type, min_thresh, max_thresh, colors);
            set(h, 'PaperPositionMode', 'auto')
            print(h, '-dpng', fullfile(cur_dir, 'output', out_name))
            close all
            % compare the output with correct output
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results fo DrawSurfaceMaps_fslr, label_data...')
                disp(['Correlation:' num2str(cor)]);
                copyfile(fullfile(cur_dir, 'output', out_name), fullfile(cur_dir, 'ref_output', out_name))
                delete(fullfile(cur_dir, 'output', out_name))
            else            
