classdef test_CBIG_DrawSurfaceMapsWithBoundary < matlab.unittest.TestCase
% Written by Nanbo Sun, Zhang Shaoshi and CBIG under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md
    ...
    methods (Test)
        function test_fsaverage_mesh(testCase)
            ...
            cur_dir = fullfile(CBIG_CODE_DIR, 'unit_tests', 'utilities', 'matlab', 'figure_utilities', 'CBIG_DrawSurfaceMapsWithBoundary');
            ...
            CBIG_DrawSurfaceMapsWithBoundary(lh_data, rh_data, lh_labels, rh_labels, mesh_name, surf_type, min_thresh, max_thresh);
        ...
        function test_fsaverage6_mesh(testCase)
            ...
            [lh_data, rh_data] = CBIG_ProjectMNI2fsaverage_Ants(mri, mesh_name);
            lh_annot = fullfile(cur_dir, 'input', mesh_name, 'lh.Yeo2011_17Networks_N1000.annot');
            rh_annot = fullfile(cur_dir, 'input', mesh_name, 'rh.Yeo2011_17Networks_N1000.annot');
            [lh_labels, ~] = CBIG_read_annotation(lh_annot);
            [rh_labels, ~] = CBIG_read_annotation(rh_annot);
            surf_type = 'sphere';
            min_thresh = 7.5e-6;
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results fo DrawSurfaceMapsInteract, fsaverage6_mesh...')
                disp(['Correlation:' num2str(cor)]);
                copyfile(fullfile(cur_dir, 'output', out_name), fullfile(cur_dir, 'ref_output', out_name))
                delete(fullfile(cur_dir, 'output', out_name))
            else            
        ...
        function test_fsaverage5_mesh(testCase)
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results fo DrawSurfaceMapsInteract, fsaverage5_mesh...')
                disp(['Correlation:' num2str(cor)]);
                copyfile(fullfile(cur_dir, 'output', out_name), fullfile(cur_dir, 'ref_output', out_name))
                delete(fullfile(cur_dir, 'output', out_name))
            else            
        ...
        function test_parcellation_input(testCase)
            ...
            CBIG_DrawSurfaceMapsWithBoundary(lh_labels, rh_labels, lh_labels, rh_labels, mesh_name, surf_type, min_thresh, max_thresh);
            set(gcf, 'PaperPositionMode', 'auto')
            print(gcf, '-dpng', fullfile(cur_dir, 'output', out_name))
            close all
            % compare the output with correct output
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results fo DrawSurfaceMapsInteract, parcellation...')
                disp(['Correlation:' num2str(cor)]);
                copyfile(fullfile(cur_dir, 'output', out_name), fullfile(cur_dir, 'ref_output', out_name))
                delete(fullfile(cur_dir, 'output', out_name))
            else            
