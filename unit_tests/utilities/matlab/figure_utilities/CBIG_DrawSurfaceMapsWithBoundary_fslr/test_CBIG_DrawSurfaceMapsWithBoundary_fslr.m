classdef test_CBIG_DrawSurfaceMapsWithBoundary_fslr < matlab.unittest.TestCase
% Written by Zhang Chen and CBIG under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md
    ...
    methods (Test)
        function test_metric_data_32k_inflated(testCase)
            ...
            cur_dir = fullfile(CBIG_CODE_DIR, 'unit_tests', 'utilities', 'matlab', 'figure_utilities', 'CBIG_DrawSurfaceMapsWithBoundary_fslr');
            ...
            CBIG_DrawSurfaceMapsWithBoundary_fslr(lh_fsLR_32k_data, rh_fsLR_32k_data, lh_fsLR_32k_label, rh_fsLR_32k_label, ...
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results fo DrawSurfaceMapsWithBoundary_fslr, metric_data_32k_inflated...')
        ...
        function test_metric_data_32k_veryinflated(testCase)
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results for DrawSurfaceMapsWithBoundary_fslr, metric_data_32k_veryInflated...')
                disp(['Correlation:' num2str(cor)]);
                copyfile(fullfile(cur_dir, 'output', out_name), fullfile(cur_dir, 'ref_output', out_name))
                delete(fullfile(cur_dir, 'output', out_name))
            else            
        ...
        function test_metric_data_32k_midthickness_orig(testCase)
            ...
            CBIG_DrawSurfaceMapsWithBoundary_fslr(lh_fsLR_32k_data, rh_fsLR_32k_data, lh_fsLR_32k_label, rh_fsLR_32k_label, fslr_mesh_name, surf_type, min_thresh, max_thresh);
            set(gcf, 'PaperPositionMode', 'auto')
            print(gcf, '-dpng', fullfile(cur_dir, 'output', out_name))
            close all
            % compare the output with correct output
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results fo DrawSurfaceMapsWithBoundary_fslr, metric_data_32k_midthickness_orig...')
                disp(['Correlation:' num2str(cor)]);
                copyfile(fullfile(cur_dir, 'output', out_name), fullfile(cur_dir, 'ref_output', out_name))
                delete(fullfile(cur_dir, 'output', out_name))
            else            
        ...
        function test_label_data(testCase)
            ...
            CBIG_DrawSurfaceMapsWithBoundary_fslr(lh_fsLR_32k_label, rh_fsLR_32k_label, lh_fsLR_32k_label, rh_fsLR_32k_label, ...
            fslr_mesh_name, surf_type, min_thresh, max_thresh);
            set(gcf, 'PaperPositionMode', 'auto')
            print(gcf, '-dpng', fullfile(cur_dir, 'output', out_name))
            close all
            ...
            if(replace_unittest_flag)
                disp('Replacing unit test reference results for DrawSurfaceMapsWithBoundary_fslr, label_data...')
                disp(['Correlation:' num2str(cor)]);
                copyfile(fullfile(cur_dir, 'output', out_name), fullfile(cur_dir, 'ref_output', out_name))
                delete(fullfile(cur_dir, 'output', out_name))
            else            
